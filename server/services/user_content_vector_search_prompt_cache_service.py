...
class UserContentVectorSearchPromptCacheService(SingletonService):
    def __init__(self):
        ...
        self.json_dir = str(settings.USER_CONTENT_QUERY_CACHE_DIR)
    ...
    def _initialize_database(self):
        ...
        c = conn.cursor()
        c.execute('''
            CREATE TABLE IF NOT EXISTS user_content_query_intent_cache (
    ...
    async def initialize(self, ai_service: AIService, vector_db_service):
        ...
        log_service.user_content("✓ User Content Cache Service Ready")
        log_service.user_content("  📂 Database:   PostgreSQL (ai_radio_embeddings)")
        log_service.user_content(f"  📂 JSON Cache: {abs_json_path}")
    ...
    def _load_cache_from_disk(self):
        ...
        c.execute("SELECT query_hash, query_text, embedding, intent_category, weights_json, "
                  ...
                  "FROM user_content_query_intent_cache")
    ...
    async def analyze_query(
        ...
        log_service.user_content(f"🧠 Checking Intent Cache for: '{query}'")
        ...
        self.gemini_calls += 1
        log_service.user_content(f"  🤖 [CACHE MISS - NEW LLM CALL] Sending '{query}' to Gemini...")
    ...
    @staticmethod
    def _cached_to_analysis(cached: Dict) -> QueryIntentAnalysis:
    ...
    def _update_cache_stats(self, query_hash: str, _match_type: str):
        if query_hash in self.query_cache:
            ...
            conn = self._get_connection()
            c = conn.cursor()
            c.execute("UPDATE user_content_query_intent_cache SET times_reused = %s, last_used = %s "
                      "WHERE query_hash = %s",
    ...
    async def _save_json_backup(self, query: str, analysis: QueryIntentAnalysis, query_hash: str):
        ...
        try:
            ...
            log_service.user_content(f"  📄 Saved JSON backup: {filepath}")
